...
const Footer = () => {
  // 🌟 الفوتر يسحب البيانات الحية بنفسه مباشرة من الـ Loader المشترك للصفحة
  ...
  return (
    <footer className="bg-[#0F2D52] text-white py-16">
      <div className="container mx-auto px-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
        ...
        {/* Our Services - ديناميكي تماماً عبر الـ useLoaderData الداخلي */}
        <div>
          <h4 className="font-[500] text-[18px] leading-[28px] text-[#2CACE3] mb-6">
            Our Services
          </h4>
          <ul className="text-gray-400 text-xs space-y-4 uppercase tracking-tighter">
            {services && services.length > 0 ? (
              services.slice(0, 5).map((service) => (
                <li key={service._id}>
                  <Link 
                    className="leading-[28px] font-heading font-[400] text-[#D1DAE6] hover:text-white transition" 
              ...
              <li className="text-gray-500 normal-case tracking-normal">No services available</li>
            )}
          </ul>
        </div>
