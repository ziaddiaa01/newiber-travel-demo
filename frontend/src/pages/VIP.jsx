...
const VIP = () => {
  const { services } = useLoaderData(); // gets the filtered VIP services
const fadeUp = {
        ...
        <div className="grid md:grid-cols-2 gap-20 items-center">
          <motion.div {...fadeUp}>
            ...
            <p className="text-gray-400 font-light leading-loose mb-10 text-sm">
              True luxury is seamless. It is felt, not seen. Our VIP division specializes in crafting journeys that flow effortlessly. From tarmac-side pickups to private island buyouts, we operate in the background so you can live in the moment.
            </p>
        ...
        <div className="px-6 mx-auto max-w-[91.9%] md:px-24">
          ...
          <div className="grid md:grid-cols-3 gap-10">
            {services.length === 0 ? (
              ...
              services.sort((a, b) => a.order - b.order).map((item, idx) => (
                <motion.div 
                  key={item._id}
                ...
                >
                  <img 
                    src={vipImagesMap[item.imageUrl] || item.imageUrl} 
...
</span>                    <h3 className="text-2xl font-light uppercase tracking-widest mb-4">{item.title}</h3>
      ...
      <section className="py-48 flex flex-col items-center justify-center text-center px-6 border-t border-white/5">
        <motion.div {...fadeUp} className="max-w-[65%]">
          ...
          <form className="w-full space-y-12">
            ...
            />
            <div className="pt-8">
              <button className="px-16 py-5 border border-[#c5a059]/40 text-[#c5a059] text-[10px] tracking-[0.5em] uppercase hover:bg-[#c5a059] hover:text-black transition-all duration-500 rounded-sm">
                Submit Request
              </button>
            </div>
          </form>
