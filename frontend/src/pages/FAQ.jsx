...
import { faqImagesMap } from '../utils/imageRegistry';

const FAQ = () => {
  const { faqs } = useLoaderData();
  
  // Handle the data structure (faqs might have .data property or be direct array)
  const faqsList = faqs?.data || faqs || [];
        ...
        {faqsList.map((faq, index) => (
          ...
          >
            ...
            <span className="text-5xl md:text-7xl font-bold text-[#184784] opacity-[25%] block mb-2">
              ...
              <h3 className="text-2xl font-black uppercase tracking-widest text-[#474747]">
                {faq.question}
              </h3>
              ...
              {/* Feature List (Optional: add to your backend schema) */}
            ...
            <div className="flex-1 w-full relative">
              <div className="aspect-[4/3] overflow-hidden rounded-sm shadow-2xl">
                <img 
                  src={faqImagesMap[faq.imageUrl] || faq.imageUrl || '/images/faq-placeholder.jpg'}
                  alt={faq.question} 
                  className="w-full h-full object-cover transition-transform duration-700 hover:scale-105" 
                />
              </div>
              {/* Optional WhatsApp Float from image 01 */}
              {index === 0 && (
                 <div className="absolute bottom-4 right-4 bg-[#25D366] p-3 rounded-full text-white shadow-lg">
